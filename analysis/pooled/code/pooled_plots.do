...
syntax, outcomes(str) time(str) num_periods(int)
	...
	foreach outcome in `outcomes' {
		...
		* Mean ES
		egen         avg_`outcome' = mean(`outcome') if (treated==1|dpto==1), by(t)
		...
		egen         avg_`outcome'_2 = mean(`outcome') if (treated==1), by(t)
		lab var      avg_`outcome'_2 "`: var lab `outcome''"
		tw connected avg_`outcome'_2 t if inrange(t,2,14), sort mc(navy) lc(navy) ///
			`opts' xtitle("`time_label'") xlabel(2 "-6" 4 "-4" 6 "-2" 8 "0" 10 "2" 12 "4" 14 "6")
